...
## Type I CPT values
The first type are the values for the CPTs of random variables Xq, that represent a conclusion of an argument, whose parents are random variables Xp1,...,Xpn , Xψ, and possibly Xα. The CPTs of such Xq encode the probability that Xq = sat or Xq = notSat , for all combinations of states of parent variables. Because we are only interested in the case when Xq = sat , each Xpi = sat , Xψ = sat , and Xα = sat , and under the assumption that the inference rule and the argument syntactically match (c.f. Section 5.2), the value P(Xq = sat |Xq = sat ,Xp1 = sat ,...,Xpn = sat ,Xψ = sound ,Xα = sat ) is set to 1. For all other combinations of states of parent variables P(Xq = sat |...) = 0.

## Type II CPT values
The second type are the values for the probability that an explicitly declared inference rule, is sound or a logical consequence of the asserted axioms. An example from (14) is P(XS1 = sound |XJ2 = sat ).
